...
class SettingsActivity : BaseActivity(R.string.settings) {
        ...
        get() {
            ...
            if (firstLocale != null) {
                for (i in 1 until languages.size) {
                    ...
                    if (firstLocale.toString().startsWith(language.isoCode)) return language
                }
            }
            return languages[0]
        }
            ...
            if (xpp.eventType == XmlPullParser.START_TAG && xpp.name == "locale") {
                languages.add(
                    Language(
                        ...
                        xpp.getAttributeValue(null, "percent")?.toInt() ?: 100
                    )
                )
            }
            xpp.next()
    ...
    private fun isExactAlarmsGranted(): Boolean {
        return Build.VERSION.SDK_INT < Build.VERSION_CODES.S || getSystemService<AlarmManager>()!!.canScheduleExactAlarms()
    }
