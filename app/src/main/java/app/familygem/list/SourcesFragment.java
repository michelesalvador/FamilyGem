...
public class SourcesFragment extends Fragment {
			...
			Collections.sort(sourceList, (f1, f2) -> {
				switch( order ) {
					case 1:	// Ordina per id numerico
						return U.getNumberOnly(f1.getId()) - U.getNumberOnly(f2.getId());
					case 2:
						return U.getNumberOnly(f2.getId()) - U.getNumberOnly(f1.getId());
	...
	public static void createNewSource(Context context, Object container) {
		Source source = new Source();
		source.setId(U.nuovoId(gc, Source.class));
		source.setTitle("");
		gc.addSource(source);
		if( container != null ) {
		...
		U.save(true, source);
		Memory.setPrimo(source);
		context.startActivity(new Intent(context, Fonte.class));
	}
	...
	public static Object[] eliminaFonte(Source source) {
		ListaCitazioniFonte citazioni = new ListaCitazioniFonte(gc, source.getId());
		for( ListaCitazioniFonte.Tripletta cita : citazioni.lista ) {
			SourceCitation cit = cita.citazione;
			cit.setRef(null);
