...
/**
 * Finds all Media with their leader object to be used by [GalleryFragment].
 * In case of shared media the leader will be the Media itself.
 * @param sharedMediaOnly Shared media only or both shared and simple media
 */
class GalleryList(private val sharedMediaOnly: Boolean) : TotalVisitor() {
    ...
    override fun visit(gedcom: Gedcom): Boolean {
        list.addAll(gedcom.media.map { MediaWrapper(it, it) }) // Collects shared media of the Gedcom
        return true
    }
    ...
    override fun visit(obj: ExtensionContainer, isLeader: Boolean): Boolean {
        return if (sharedMediaOnly) false
            ...
            if (obj is MediaContainer) {
                list.addAll(obj.media.map { MediaWrapper(it, leader) }) // Collects simple media of the object
            }
            true
        }
    }

    data class MediaWrapper(val media: Media, val leader: NoteContainer) {
        var fileUri: FileUri? = null
        var text = "" // All relevant field values concatenated
        private val fields: MutableList<String?> = mutableListOf(media.title, media.file, media.format)
